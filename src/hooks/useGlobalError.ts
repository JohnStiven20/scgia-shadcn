...
function getStatusMessage(status?: number): string {
  ...
  return (
    ...
    `Se produjo un error en la solicitud (${status}).`
  )
}
...
function buildErrorMessage(error: unknown, fallbackMessage: string): string {
  ...
  if (isFetchBaseQueryError(error)) {
    ...
    const baseQueryMessage =
      "error" in error ? normalizeText(error.error) : null
  ...
  if (error && typeof error === "object" && "message" in error) {
    return (
      normalizeText((error as SerializedLikeError).message) || fallbackMessage
    )
  }
